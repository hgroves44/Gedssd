...
void EventAction::EndOfEventAction(const G4Event*) {
    ...
    FillAncillaryBgo() ;
    FillSceptar() ;
    FillGedssd() ;//Added to incorporate gedssd
    FillGridCell() ;
...
void EventAction::ClearVariables() {
	...
	//Add this as well for gedssd
	fGedssdEnergyDet[i]  = 0 ;
	fGedssdTrackDet[i]   = 0 ;
...
//This is put in for the gedssd geometry
void EventAction::FillGedssd() {
    G4double energySumDet = 0;
    for (G4int j=0; j < MAXNUMDET; j++) {
	if(fGedssdEnergyDet[j] > MINENERGYTHRES) {
	    if(WRITEEDEPHISTOS)     HistoManager::Instance().FillHisto(kGedssdEdep, fGedssdEnergyDet[j]);
	    if(WRITEEDEPHISTOS)     HistoManager::Instance().FillHisto(kGedssdEdepDet0+j, fGedssdEnergyDet[j]);
	    energySumDet += fGedssdEnergyDet[j];
	}
    }
    if(energySumDet > MINENERGYTHRES) {
	if(WRITEEDEPHISTOS)      HistoManager::Instance().FillHisto(kGedssdEdepSum, energySumDet);
    }
