...
void SteppingAction::UserSteppingAction(const G4Step* aStep) {
    ...
    //This below is being added to try and incoroporate the gedssd and clover. I will be modeling it after Descant and Testcan. I am unsure how the numbering works, but I am just going to choose something no one else has I suppose. 
    //
    found = volname.find("GeThinDetector"); 
    if ( found!=G4String::npos) {
	G4cout << "The thin detector loop works"  << G4endl;
	SetDetNumberForGenericDetector(volname);
	fEventAction->GedssdDet(edep, stepl, fDet-1);//I may not need this but copied it from other detectors.
	mnemonic.replace(0,3,"GTN");
	mnemonic.replace(3,2,G4intToG4String(fDet));
	mnemonic.replace(5,1,GetCrystalColour(fCry));
	systemID = 9500;
	fEventAction->AddHitTracker(mnemonic, evntNb, trackID, parentID, fStepNumber, particleType, processType, systemID, fCry-1, fDet-1,  edep, pos2.x(), pos2.y(), pos2.z(), time2, targetZ);
    ...
    found = volname.find("GeThickDetector");
    if (edep !=0 && found!=G4String::npos) {
	G4cout << "The Thick Detector loop works" << G4endl;
	SetDetNumberForGenericDetector(volname);
	fEventAction->GedssdDet(edep, stepl, fDet-1);
	mnemonic.replace(0,3,"GTK");
	mnemonic.replace(3,2,G4intToG4String(fDet));
	mnemonic.replace(5,1,GetCrystalColour(fCry));
	systemID = 9600;
	fEventAction->AddHitTracker(mnemonic, evntNb, trackID, parentID, fStepNumber, particleType, processType, systemID, fCry-1, fDet-1, edep, pos2.x(), pos2.y(), pos2.z(), time2, targetZ);
    ...
    found = volname.find("CloverCrystal");
    if (edep !=0 && found!=G4String::npos) {
	G4cout << "The Clover loop works" << G4endl;
	SetDetNumberForGenericDetector(volname);
	fEventAction->GedssdDet(edep, stepl, fDet-1);
	mnemonic.replace(0,3,"CCL");
	mnemonic.replace(3,2,G4intToG4String(fDet));
	mnemonic.replace(5,1,GetCrystalColour(fCry));
	systemID = 9700;
	fEventAction->AddHitTracker(mnemonic, evntNb, trackID, parentID, fStepNumber, particleType, processType, systemID, fCry-1, fDet-1, edep, pos2.x(), pos2.y(), pos2.z(), time2, targetZ);
