...
G4int DetectionSystemGedssd::PlaceDetector(G4LogicalVolume* expHallLog, G4int detectorNumber) {
    ...
    for(G4int i=1; i<4; i++) {
	//I am attempting to try and add another detector in the visualization. For now, I am simply going to try and do a simple multiplication. 
	//(5/24/17) This appeared to have adjusted the rotation and added another detector, but to be sure, I will try adding a third detector and adjust angles
	rotate->rotateY(i*45*deg);
